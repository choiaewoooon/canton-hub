...
### 임계값 근거 (리서치 확인, 2026-06-01)

업계 통념상 **mNAV의 구조적 임계선은 정확히 1.0x 하나뿐**이다 — 1.0x 위는 프리미엄(신주 발행이 accretive, 자금조달 플라이휠 작동), 1.0x 아래는 디스카운트(신주 발행이 희석적 → 자금조달 차단 → death spiral 시작). 이론적으로 합의된 "1.2x" 같은 상단 안전 밴드는 **존재하지 않는다**(River, Architect, AInvest, NYDIG 등 다수 출처 확인 — 모두 1.0x만 명시, 그 위 watch/healthy 구분은 미정의).

→ 따라서 **1.0x = 절대 기준선은 유지**(이론적으로 단단함). 그 위의 **1.2x = "1.0x에 너무 가까우면 경고"용 휴리스틱 완충 밴드**일 뿐 이론적 최적값이 아니다. 구현 시 이 1.2x 경계는 **상수 하나로 분리**(`MNAV_WATCH_THRESHOLD = 1.2`)해서 나중에 쉽게 조정할 수 있게 둔다. 데이터가 쌓이면 CNTN의 실제 mNAV 변동폭을 보고 재튜닝.
